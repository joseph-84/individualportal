...
| 할일 · 일정 | 계층 구조(하위 할일 — 리스트·칸반 모두 진행률 바 표시, 칸반 카드는 지라 스타일 체크리스트도 함께 표시, 칸반에서도 하위 할일 생성 가능; 모든 하위 할일이 완료되면 상위 할일도 자동으로 완료 처리, 필요하면 다시 수동으로 되돌릴 수 있음), **상태 순환 배지**(리스트·칸반 체크리스트에서 클릭 시 할 일 → 진행중 → 완료 → 할 일 순으로 전환), **리치 텍스트 설명**(굵게/기울임/불렛·번호 목록/체크박스, Tiptap 기반 — 체크박스는 리스트·칸반 화면에서도 편집 모드 없이 바로 체크 가능), 수정, **드래그 앤 드롭으로 순서 변경**(⠿ 손잡이를 잡았을 때만 동작 — 텍스트 영역은 그대로 선택/복사 가능), 리스트/**칸반**/캘린더 뷰, **태그·프로젝트·기간별 필터** + **제목 텍스트 검색**(이전 티켓 찾기 용도, 하위 할일 제목까지 검색), 리스트 뷰에는 "숨겨진 항목 보기" 필터도 추가. 완료된 지 2주가 지난 할일은 칸반에서 항상 숨겨지고 리스트에서도 기본적으로 숨겨짐(해당 필터 또는 검색 중일 때 다시 볼 수 있음), 마지막으로 본 뷰 기억(브라우저 단위), 연동된 Google Calendar 일정 표시(읽기 전용) |
| 지식베이스 | 폴더/태그 기반 위키, 마크다운(.md) 또는 HTML(.html)로 문서 생성, 실시간 검색, 뷰어·에디터. HTML 문서는 Tiptap 기반 **GUI 위지위그 편집기**로 작성(제목/서식/목록/체크박스/인용/코드/링크/이미지/표/정렬 — SVG 등 완전 자유 형식 HTML은 범위 밖). 마크다운 문서는 원시 소스 편집 + 실시간 미리보기(모바일에서는 미리보기가 하단 모달로 분리되고 보조 툴바 버튼은 숨겨져 간소화됨) |
| 체크리스트 | 폴더 기반 체크리스트 문서 생성·관리(지식베이스와 별도 페이지), Tiptap 기반 위지위그 에디터로 제목/체크박스 목록 작성. **체크박스는 에디터 모드에 들어가지 않아도 뷰어 화면에서 바로 체크/해제하고 즉시 저장됨**(할일 설명 체크박스와 동일한 방식) |
| 파일 브라우저 | `SYNC_FOLDER_PATH` 실제 서버 폴더를 **파일탐색기 스타일 트리**로 조회(하위 폴더 지연 로딩)/**드래그 앤 드롭 업로드**(다중 파일)/다운로드/폴더 생성/삭제/이름변경/**이동·복사**(파일·폴더 모두, 대상 폴더를 트리에서 선택), 지식베이스 문서를 가상 폴더로 열람(읽기 전용), 파일 선택 시 목록이 좌측으로 축소되며 **대형 미리보기** 표시 + **새 창에서 보기**(.md/.html은 위키 뷰어와 동일하게 렌더링된 문서로 표시), **파일 공유 링크 생성** |
| 즐겨찾기 | 자주 쓰는 문서·URL을 폴더로 묶어 저장, 폴더 트리 형태의 간결한 목록(접기/펼치기 가능), 클릭 시 항상 **새 창**으로 열림, **드래그 앤 드롭으로 순서·폴더 이동**(⠿ 손잡이) |
...
자주 찾는 문서나 URL을 제목·주소·폴더로 저장해두는 북마크 목록입니다(`/favorites`). 폴더 트리 형태의 간결한 목록으로 표시되며(폴더는 클릭해서 접고 펼칠 수 있음), 검색으로 빠르게 찾을 수 있습니다. 항목을 클릭하면 항상 **새 창(탭)**으로 열립니다 — 포털 화면을 벗어나지 않고 참고 링크를 열어볼 수 있습니다. ⠿ 손잡이를 드래그해 같은 폴더 내 순서를 바꾸거나 다른 폴더로 옮길 수 있습니다(검색 중에는 비활성화). URL에 `http://`/`https://` 등 스킴을 생략하면(`example.com`처럼 입력) 자동으로 `https://`를 붙입니다. 외부 URL뿐 아니라 포털 내부 경로(예: `/wiki?id=...`, `/files/view?path=...`)도 그대로 등록할 수 있습니다.
...
| 분류 | 도구 | 설명 |
|---|---|---|
| 할일 | `list_todos` | 할일 목록 조회 (상태/상위 할일 ID로 필터) |
| | `create_todo` | 할일 생성 (하위 할일은 `parentId` 지정, `status` 기본값 `todo`) |
| | `update_todo` | 제목·설명·프로젝트·태그·반복·마감일·상태(`todo`/`in_progress`/`done`) 수정 |
...
- 반응형 레이아웃(사이드바 접기, 좁은 화면에서 컬럼 쌓기 등)은 적용되어 있지만, 실제 모바일 기기에서의 테스트나 자동화 테스트(단위/E2E)는 아직 없습니다.
