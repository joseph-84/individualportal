...
| 할일 · 일정 | 계층 구조(하위 할일 — 리스트·칸반 모두 진행률 바 표시, 칸반 카드는 지라 스타일 체크리스트도 함께 표시, 칸반에서도 하위 할일 생성 가능; 모든 하위 할일이 완료되면 상위 할일도 자동으로 완료 처리, 필요하면 다시 수동으로 되돌릴 수 있음), **상태 순환 배지**(리스트·칸반 체크리스트에서 클릭 시 할 일 → 진행중 → 완료 → 할 일 순으로 전환), **리치 텍스트 설명**(굵게/기울임/불렛·번호 목록/체크박스, Tiptap 기반 — 체크박스는 리스트·칸반 화면에서도 편집 모드 없이 바로 체크 가능), 수정, **드래그 앤 드롭으로 순서 변경**(⠿ 손잡이를 잡았을 때만 동작 — 텍스트 영역은 그대로 선택/복사 가능), 리스트/**칸반**/캘린더 뷰, **태그·프로젝트·기간별 필터**, 마지막으로 본 뷰 기억(브라우저 단위), 연동된 Google Calendar 일정 표시(읽기 전용) |
