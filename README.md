...
| 할일 · 일정 | 계층 구조(하위 할일), **리치 텍스트 설명**(굵게/기울임/불렛·번호 목록/체크박스, Tiptap 기반 — 체크박스는 리스트·칸반 화면에서도 편집 모드 없이 바로 체크 가능), 수정, 상태(할 일/진행중/완료), **드래그 앤 드롭으로 순서 변경**(⠿ 손잡이를 잡았을 때만 동작 — 텍스트 영역은 그대로 선택/복사 가능), 리스트/**칸반**/캘린더 뷰, **태그·프로젝트·기간별 필터**, 마지막으로 본 뷰 기억(브라우저 단위), 연동된 Google Calendar 일정 표시(읽기 전용) |
