...
export function ShareDialog({ relPath, onClose }: { relPath: string; onClose: () => void }) {
  ...
  const [scope, setScope] = useState<"members" | "public">("members");
  ...
  const load = () => {
    startTransition(async () => {
      ...
      setLinks(res.map((l) => ({ id: l.id, token: l.token, scope: l.scope, createdAt: l.createdAt.toString() })));
    });
  };
  ...
  const create = () => {
    setError(null);
    startTransition(async () => {
      const res = await createShareLinkAction(relPath, scope);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.35)", display: "grid", placeItems: "center", zIndex: 50 }} onClick={onClose}>
      <div
        ...
        style={{ width: 420, maxWidth: "90vw", background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 12, padding: 18, boxShadow: "0 16px 40px rgba(0,0,0,.25)" }}
      >
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          <select value={scope} onChange={(e) => setScope(e.target.value as "members" | "public")} style={{ flex: 1, height: 32, border: "1px solid var(--line)", borderRadius: 7, background: "var(--panel2)", color: "var(--ink)", fontSize: 12.5 }}>
            <option value="members">회원 대상 (로그인 필요)</option>
        ...
        <div style={{ display: "grid", gap: 6, maxHeight: 220, overflow: "auto" }}>
          ...
          {links?.map((l) => (
            <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 9px", border: "1px solid var(--line)", borderRadius: 7, background: "var(--panel2)" }}>
              <span style={{ fontSize: 9.5, fontWeight: 700, padding: "1px 6px", borderRadius: 4, background: l.scope === "public" ? "var(--warn-soft)" : "var(--ok-soft)", color: l.scope === "public" ? "var(--warn)" : "var(--ok)", flex: "none" }}>
                {l.scope === "public" ? "전체공개" : "회원대상"}
              </span>
              <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11.5, fontFamily: "var(--font-mono), monospace", color: "var(--ink2)" }}>
                /share/{l.token}
              </span>
